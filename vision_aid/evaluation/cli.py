...
from vision_aid.evaluation.audit import (
    CONFIGURATION, MODEL, REQUEST_CONFIG, execute, positive_cost, write_json,
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run = commands.add_parser("run", help="preview or execute the fixed Haiku experiment")
    ...
    run.add_argument("--max-cost-usd", required=True, help="positive between-request limit")
    ...
    report = commands.add_parser("report", help="score completed run and edited review.csv")
    report.add_argument("--run-dir", type=Path, required=True)
    return parser


def _new_run_directory() -> Path:
    root = Path.cwd() / ".model-evaluation" / "runs"
...
def _run(args: argparse.Namespace) -> int:
    ...
    production = run_pipeline(
        html_path=str(run / "snapshot.html"), output_dir=run, api_key=None,
        model=MODEL, dry_run=True, include_summaries=False,
    ...
    findings = normalize_programmatic_findings(
        raw_programmatic, run_id=run.name, page_url=args.source_url,
    )
    ...
    manifest = {
        "run_id": run.name, "source_url": args.source_url,
        "workbook_filename": references.workbook_filename,
        "workbook_sha256": references.workbook_sha256,
        "snapshot_sha256": snapshot_hash,
        "configuration": CONFIGURATION,
        "pricing_version": schedule.version, "pricing_sha256": schedule.identity,
        "max_cost_usd": str(limit), "complete": False,
        "incomplete_reason": "preview", "estimated_cost_usd": "0",
        "usage": {key: 0 for key in (
            "input_tokens", "cached_input_tokens",
            "cache_creation_input_tokens", "output_tokens",
        )},
        "format_failures": [],
        "prompts": [{"name": entry["name"], "checklist": entry["checklist"]}
    ...
    print(f"Model: {MODEL}; endpoint: Messages; thinking: 16000; output cap: 24192")
    print(f"Temperature: omitted; summaries: disabled; prompts: {len(manifest['prompts'])}")
    print(f"Snapshot SHA-256: {snapshot_hash}; Reference rows: {len(references.references)}")
    ...
    if not args.approve_live:
        if not sys.stdin.isatty() or input("Type 'yes' to approve paid execution: ") != "yes":
...
def _report(args: argparse.Namespace) -> int:
    ...
    for kind, collection in (("audit", audit), ("programmatic", programmatic)):
        if any(item["run_id"] != run.name or item["source"] != kind or
               item["page_url"] != manifest["source_url"] for item in collection):
            raise ValueError(f"{kind} findings do not belong to this run")
    rows = reviewed_rows(run / "review.csv", reference_set["references"],
                         audit, programmatic)
