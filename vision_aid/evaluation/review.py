...
DECISION_COLUMNS = (
    "classification", "classification_reason", "audit_finding_id",
    "programmatic_finding_id", "match_reason", "review_notes",
)
REVIEW_COLUMNS = (
    "reference_id", "source_sheet", "source_row", "page_scope",
    "source_element", *HEADERS, *DECISION_COLUMNS,
)
...
def reviewed_rows(
    path: Path, references: list[dict[str, Any]],
    audit: list[dict[str, Any]], programmatic: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    ...
    for row in entered:
        ...
        for field in ("source_sheet", "source_row", "source_element"):
            if str(row[field]) != str(reference[field]):
                raise ValueError(f"Reference identity changed for {reference_id}: {field}")
        ...
        for kind, column in (
            ("audit", "audit_finding_id"), ("programmatic", "programmatic_finding_id")
        ):
            ids = _ids(row[column])
            unknown = set(ids) - set(by_kind[kind])
            if unknown:
                raise ValueError(f"Unknown {kind} finding ID: {', '.join(sorted(unknown))}")
            reused = set(ids) & used[kind]
            if reused:
                raise ValueError(f"Conflicting {kind} finding ID: {', '.join(sorted(reused))}")
        ...
        if classification in {"unavailable_evidence", "ambiguous"} and any(matched.values()):
...
def write_report(
    ...
    checks = [row for row in rows if row["decision"]["classification"] == "programmatic"]
    covered = [row for row in rows if any(row["matches"].values())]
    unavailable = sum(row["decision"]["classification"] == "unavailable_evidence" for row in rows)
    ...
    lines = [
        ...
        f"- Workbook-row recall: {_metric(sum(bool(row['matches']['audit']) for row in llm), len(llm))}",
        f"- Programmatic coverage: {_metric(sum(bool(row['matches']['programmatic']) for row in checks), len(checks))}",
        ...
        "", "This is full-row coverage of this homepage workbook only. A blank finding cell means missed.",
        "The cost guardrail is checked between requests; a request can exceed the remaining amount.",
        f"Malformed response prompts: {_md(', '.join(manifest['format_failures']) or 'none')}",
        "", "## Reference defects", "",
    ]
    for row in rows:
        ...
        lines.extend([
            ...
            f"- Source: {reference['source_sheet']} row {reference['source_row']} ({reference['page_scope']})",
            f"- source_element: {_md(reference['source_element'])}",
            f"- Original defect evidence: {_md(json.dumps(reference['raw_evidence'], ensure_ascii=False))}",
            f"- Classification: {decision['classification']} — {_md(decision['classification_reason'])}",
            f"- Audit findings: {_md('; '.join(matches['audit']) or 'none')}",
            f"- Programmatic findings: {_md('; '.join(matches['programmatic']) or 'none')}",
        ...
        for finding_id in (*matches["audit"], *matches["programmatic"]):
            ...
            lines.append(f"- {finding_id}: {_md(finding['problem'])} @ {_md(finding['location'])}")
        lines.append("")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
